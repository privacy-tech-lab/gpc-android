...
class MainActivity : WebViewExtActivity(), SearchBarController.OnCancelListener,
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        PreferenceManager.getDefaultSharedPreferences(this).apply {
            // Check if we need to display DNT Preference Dialog
            if (!getBoolean("COMPLETED_DNT_DIALOG", false)) {
                // The user hasn't seen the DNT Preference Dialog, so show it to them
                GPCPreferenceDialogFragment().show(supportFragmentManager, "DNTDialog")
                //set sharedPreference to indicate dialog has been seen
                PreferenceManager.getDefaultSharedPreferences(baseContext).edit().apply {
                    putBoolean("COMPLETED_DNT_DIALOG", true)
                    apply()
                }
            }
        }
